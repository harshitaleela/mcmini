...
main(int argc, char *argv[])
{
  ...
  while (cur_arg[0] != NULL && cur_arg[0][0] == '-') {
    if (strcmp(cur_arg[0], "--max-transition-depth") == 0 ||
        strcmp(cur_arg[0], "-m") == 0) {
      setenv(ENV_MAX_TRANSITION_DEPTH, cur_arg[1], 1);
      char *endptr;
      if (strtol(cur_arg[1], &endptr, 10) == 0 && endptr[0] != '\0') {
        fprintf(stderr, "%s: illegal value\n", "--max-transition-depth");
        exit(1);
      }
      cur_arg += 2;
    }
    else if (cur_arg[0][1] == 'm' && isdigit(cur_arg[0][2])) {
      setenv(ENV_MAX_TRANSITION_DEPTH, cur_arg[0] + 2, 1);
      cur_arg++;
    }
      ...
      fprintf(stderr, "Usage: mcmini [--max-transition-depth|-m <num>]\n"
